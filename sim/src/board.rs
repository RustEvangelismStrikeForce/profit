...
/// Places a building without checking some things
///
/// Panics if the connections are invalid
pub fn place_building_unchecked(sim: &mut Sim, building: Building) -> Id {
    ...
    match &building {
        Building::Deposit(deposit) => {
            ...
            for y in (pos.y)..(pos.y + height) {
                for x in (pos.x)..(pos.x + width) {
                    place_cell_unchecked(sim, (x, y), Cell::output(id));
                }
            }

            for x in (pos.x)..(pos.x + width) {
                check_adjacent_cells(sim, (x, pos.y), (x, pos.y - 1))
                    .expect("Expected building to be valid");
            }
            for y in (pos.y)..(pos.y + height) {
        ...
        Building::Obstacle(obstacle) => {
            ...
            for y in 0..height {
                for x in 0..width {
                    place_cell_unchecked(sim, pos + (x as i8, y as i8), Cell::inert(id));
                }
            }
        }
        Building::Mine(mine) => {
            ...
            for (p, ty) in MINE_CELLS[rot] {
                place_cell_unchecked(sim, pos + p, Cell::new(ty, id));
            }
            for (a, b) in ADJACENT_MINE_CELLS[rot] {
                check_adjacent_cells(sim, pos + a, pos + b).expect("Expected building to be valid");
            }
        }
        ...
        Building::Factory(factory) => {
            ...
            for y in (pos.y)..(pos.y + FACTORY_SIZE) {
                for x in (pos.x)..(pos.x + FACTORY_SIZE) {
                    place_cell_unchecked(sim, (x, y), Cell::input(id));
                }
            }
            for x in (pos.x)..(pos.x + FACTORY_SIZE) {
                check_adjacent_cells(sim, (x, pos.y), (x, pos.y - 1))
...
fn place_cell_unchecked(sim: &mut Sim, pos: impl Into<Pos>, cell: Cell) {
    let pos = pos.into();
